...
class FixSamsungTouchTracker : TouchTracker {
        ...
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                estimatedScaling = runCatching {
                    PointF(
                        touchDownAbsolutePosition.x / touchDownRawAbsolutePosition.x,
                        touchDownAbsolutePosition.y / touchDownRawAbsolutePosition.y
                    )
                }.getOrDefault(PointF(1.0f, 1.0f))
            }
                ...
                iteratePointerIndexes(event)
                    .forEach { (id, index) ->
                        ...
                        currentPositions[id] = PointF(
                            touchDownRelativePosition.x + deltaX * estimatedScaling.x,
                            touchDownRelativePosition.y + deltaY * estimatedScaling.y
                        )
                    }
            }
        }
